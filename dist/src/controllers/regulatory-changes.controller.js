...
async function generateActionPlan(req, res) {
    try {
        ...
        const { generateComplianceActionPlan } = await Promise.resolve().then(() => __importStar(require('../../services/compliance/regulatory-change-impact-analysis')));
        const actionPlan = await generateComplianceActionPlan(impact_analysis, regulatory_change, organization_capacity, userId);
        ...
        if (totalHighPriorityActions > 10)
            estimatedTimelineWeeks = 16;
        else if (totalHighPriorityActions > 5)
            estimatedTimelineWeeks = 12;
        else if (totalHighPriorityActions > 2)
...
async function estimateEffort(req, res) {
    try {
        ...
        const { estimateComplianceEffort } = await Promise.resolve().then(() => __importStar(require('../../services/compliance/regulatory-change-impact-analysis')));
        const effortEstimation = await estimateComplianceEffort(validatedChanges, validatedCapacity, userId);
    ...
    try {
        ...
        for (const systemId of system_ids) {
            try {
                ...
                const { analyzeRegulatoryImpact } = await Promise.resolve().then(() => __importStar(require('../../services/compliance/regulatory-change-impact-analysis')));
                const impactAnalysis = await analyzeRegulatoryImpact(regulatory_change, analysisSystemData, userId);
                systemAnalyses.push({
                    system_id: systemId,
        ...
        let overallPortfolioImpact = 'minimal';
        if (impactDistribution.critical > 0)
            overallPortfolioImpact = 'critical';
        else if (impactDistribution.high > 0)
            overallPortfolioImpact = 'high';
        else if (impactDistribution.medium > 0)
            overallPortfolioImpact = 'medium';
        else if (impactDistribution.low > 0)
            overallPortfolioImpact = 'low';
        ...
        const prioritySystems = systemAnalyses
            ...
            impact_level: s.impact_analysis.overall_impact_level,
            key_concerns: s.impact_analysis.affected_areas.slice(0, 3).map(area => area.area),
            estimated_effort: s.impact_analysis.affected_areas.length > 0 ? s.impact_analysis.affected_areas[0].estimated_effort : 'medium'
        }));
        return res.status(200).json({
            regulatory_change: {
                title: regulatory_change.title,
...
//# sourceMappingURL=regulatory-changes.controller.js.map