...
exports.getSystemPolicies = getSystemPolicies;
exports.postSystemPolicy = postSystemPolicy;
exports.getSystemRiskAssessments = getSystemRiskAssessments;
exports.postSystemRiskAssessment = postSystemRiskAssessment;
exports.postRiskMitigations = postRiskMitigations;
exports.getRiskTrends = getRiskTrends;
exports.postSmartRiskAssessment = postSmartRiskAssessment;
exports.postTransitionPlan = postTransitionPlan;
exports.postTransitionReadiness = postTransitionReadiness;
exports.postDocumentation = postDocumentation;
...
const risk_assessment_1 = require("../../services/risk-assessment");
const smart_assessment_1 = require("../../services/risk-assessment/smart-assessment");
const smart_lifecycle_transition_2 = require("../../services/governance/smart-lifecycle-transition");
/**
...
async function postBlockerResolutions(req, res) {
    try {
        ...
        const validatedBlockers = blockers.map((blocker, index) => {
        ...
        const blockerResolutions = await (0, smart_lifecycle_transition_1.suggestBlockerResolutions)(validatedBlockers, systemContext, userId);
        ...
        const resolutionMetrics = {
            ...
            strategies_by_effort: {
                low: blockerResolutions.reduce((sum, resolution) => sum + resolution.resolution_strategies.filter(s => s.estimated_effort === 'low').length, 0),
                medium: blockerResolutions.reduce((sum, resolution) => sum + resolution.resolution_strategies.filter(s => s.estimated_effort === 'medium').length, 0),
                high: blockerResolutions.reduce((sum, resolution) => sum + resolution.resolution_strategies.filter(s => s.estimated_effort === 'high').length, 0)
            }
        };
        ...
        const technicalProcessBlockers = blockerResolutions.filter(r => r.blocker_type === 'technical' || r.blocker_type === 'process');
        ...
        const docGovernanceBlockers = blockerResolutions.filter(r => r.blocker_type === 'documentation' || r.blocker_type === 'governance');
...
const openai = OPEN_AI_KEY ? new openai_1.OpenAI({ apiKey: OPEN_AI_KEY }) : null;
/**
...
async function getOverallRisk(req, res) {
    try {
        ...
        const overallRisk = (0, risk_assessment_1.calculateOverallRiskLevel)(assessments || []);
...
async function postRiskMitigations(req, res) {
    try {
        ...
        const mitigationSuggestions = await (0, smart_assessment_1.suggestRiskMitigations)(risk_factors, systemContext, userId);
...
async function getRiskTrends(req, res) {
    try {
      ...
      `)
        ...
        const trendAnalysis = await (0, smart_assessment_1.analyzeRiskTrends)(systemId, systemHistory, industryBenchmarks, userId);
...
async function postTransitionPlan(req, res) {
    try {
        ...
        const transitionPlan = await (0, smart_lifecycle_transition_2.generateTransitionPlan)(planningSystemData, from_stage, to_stage, readiness_assessment, userId);
        ...
        const planMetrics = {
            ...
            activities_by_priority: {
                immediate: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.priority === 'immediate').length, 0),
                high: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.priority === 'high').length, 0),
                medium: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.priority === 'medium').length, 0),
                low: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.priority === 'low').length, 0)
            },
            activities_by_category: {
                regulatory: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.category === 'regulatory').length, 0),
                technical: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.category === 'technical').length, 0),
                process: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.category === 'process').length, 0),
                documentation: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.category === 'documentation').length, 0),
                governance: transitionPlan.phases.reduce((sum, phase) => sum + phase.activities.filter(a => a.category === 'governance').length, 0)
            },
            risks_by_impact: {
                high: transitionPlan.phases.reduce((sum, phase) => sum + phase.risks.filter(r => r.impact === 'high').length, 0),
                medium: transitionPlan.phases.reduce((sum, phase) => sum + phase.risks.filter(r => r.impact === 'medium').length, 0),
                low: transitionPlan.phases.reduce((sum, phase) => sum + phase.risks.filter(r => r.impact === 'low').length, 0)
            }
        };
...
async function postTransitionReadiness(req, res) {
    try {
        ...
        const readinessAssessment = await (0, smart_lifecycle_transition_2.assessTransitionReadiness)(assessmentSystemData, target_stage, userId);
...
async function postDocumentation(req, res) {
    try {
        ...
        const { id: systemId } = req.params;
        const body = req.body;
        let { regulation_type, document_type } = body;
        ...
        let riskAssessmentsQuery = client_1.supabase
            .from("risk_assessments")
            .select("*")
            .eq("ai_system_id", systemId);
        ...
        const documentation = await generateDocumentation(regulation_type, document_type, systemData, riskAssessments || [], {
            aiSystemVersion,
...
async function getComplianceData(req, res) {
    try {
        ...
        const lifecycleStage = euResult.data?.lifecycle_stage ||
            masResult.data?.lifecycle_stage ||
            ukResult.data?.lifecycle_stage ||
            'Draft';
        ...
        if (euResult.data) {
            systemInfo = {
                type: "EU AI Act",
                name: euResult.data.system_name || "Unnamed System",
                id: systemId,
                created_at: euResult.data.created_at,
                accountable_person: euResult.data.accountable_person || "Not specified",
            ...
            systemInfo = {
                type: "MAS",
                name: masResult.data.system_name || "Unnamed System",
                id: systemId,
                created_at: masResult.data.created_at || new Date().toISOString(),
                accountable_person: masResult.data.owner || "Not specified",
                lifecycle_stage: lifecycleStage,
            };
        }
            ...
            systemInfo = {
                type: "UK AI Act",
                name: ukResult.data.system_name || "Unnamed System",
                id: systemId,
                created_at: ukResult.data.created_at || new Date().toISOString(),
                accountable_person: ukResult.data.accountable_person || "Not specified",
                lifecycle_stage: lifecycleStage,
            };
        }
        return res.status(200).json({
 ...
 */
async function generateDocumentation(regulationType, documentType, systemData, riskAssessments, traceability) {
    ...
    try {
        ...
        regulatoryContext = await (0, rag_service_1.getRegulationContextString)(ragQuery, ragRegulationType, 8);
 ...
 */
function buildPrompt(regulationType, documentType, systemData, riskAssessments, traceability, regulatoryContext) {
    ...
    const getRiskAssessmentStatusText = (status) => {
    ...
    if (riskAssessments.length > 0) {
        riskSummary = riskAssessments.map(ra => {
            ...
            if (ra.risk_score)
                line += ` [Risk Score: ${ra.risk_score}]`;
            if (ra.assessed_at)
                line += ` [Assessed: ${new Date(ra.assessed_at).toLocaleDateString()}]`;
            return line;
        }).join('\n');
    }
...
function buildMASPrompt(systemData, riskSummary) {
...
function buildModelCardPrompt(regulationType, systemData, riskSummary) {
...
function buildTechnicalDocPrompt(regulationType, systemData, riskSummary) {
    // Extract EU AI Act specific data
    const rawAnswers = systemData.raw_answers || {};
    const systemDescription = rawAnswers.q2 || systemData.system_description || systemData.description || '';
    const isEU = regulationType === 'EU AI Act';
...
function buildDPIAPrompt(regulationType, systemData, riskSummary) {
    // Extract EU AI Act specific data
    const rawAnswers = systemData.raw_answers || {};
    const systemDescription = rawAnswers.q2 || systemData.system_description || systemData.description || '';
    const isEU = regulationType === 'EU AI Act';
...
function buildRiskReportPrompt(regulationType, systemData, riskSummary) {
    // Extract EU AI Act specific data
    const rawAnswers = systemData.raw_answers || {};
    const systemDescription = rawAnswers.q2 || systemData.system_description || systemData.description || '';
    const isEU = regulationType === 'EU AI Act';
...
function buildAlgorithmImpactPrompt(regulationType, systemData, riskSummary) {
    // Extract EU AI Act specific data
    const rawAnswers = systemData.raw_answers || {};
    const systemDescription = rawAnswers.q2 || systemData.system_description || systemData.description || '';
    const isEU = regulationType === 'EU AI Act';
...
function buildAuditTrailPrompt(regulationType, systemData, riskSummary) {
    // Extract EU AI Act specific data
    const rawAnswers = systemData.raw_answers || {};
    const systemDescription = rawAnswers.q2 || systemData.system_description || systemData.description || '';
    const isEU = regulationType === 'EU AI Act';
...
//# sourceMappingURL=ai-systems.controller.js.map