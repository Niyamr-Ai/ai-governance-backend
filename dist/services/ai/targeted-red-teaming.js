 ...
 */
async function generateTargetedTests(systemId, userId, attackTypes, testCount = 5) {
 ...
 */
async function getSystemContext(systemId, userId) {
    try {
        ...
        const context = await (0, user_system_rag_service_1.getUserSystemContextString)(query, userId, 5, systemId); // TEMPORARY: using userId as orgId during transition
        if (context &&
 ...
 */
async function getAttackMethodologies(attackTypes) {
    try {
        ...
        const context = await (0, platform_rag_service_1.getPlatformContextString)(query, 5, 'red-teaming');
        if (context &&
 ...
 */
function analyzeSystemRiskProfile(systemContext) {
    const riskFactors = [];
 ...
 * Build test generation strategy
 */
function buildTestStrategy(systemId, systemContext, riskProfile, attackTypes, testCount = 5) {
    ...
    const focusAttackTypes = attackTypes || ['prompt_injection', 'jailbreak', 'data_leakage', 'policy_bypass'];
    return {
        systemId,
 ...
 */
async function generateSystemSpecificAttacks(strategy, systemContext, attackMethodologies) {
    const attacks = [];
    ...
    for (const attackType of strategy.attackTypes) {
        ...
        for (const baseAttack of attacksToCustomize) {
            try {
                const targetedAttack = await customizeAttackForSystem(baseAttack, strategy, systemContext, attackMethodologies);
 ...
 */
async function customizeAttackForSystem(baseAttack, strategy, systemContext, attackMethodologies) {
 ...
 */
function buildCustomizationPrompt(baseAttack, strategy, systemContext, attackMethodologies) {
 ...
 */
async function generateTestRecommendations(strategy, systemContext) {
 ...
 * Generate risk assessment
 */
async function generateRiskAssessment(strategy, systemContext) {
    ...
    // Overall risk level
    const highRiskFactors = riskFactors.filter(r => r.includes('high_risk') || r.includes('critical') || r.includes('production')).length;
 ...
 */
function extractSystemName(systemContext) {
    if (!systemContext.hasSystemData) {
 ...
 */
function extractSystemType(systemContext) {
    if (!systemContext.hasSystemData) {
        return 'AI System';
    }
    const context = systemContext.rawContext.toLowerCase();
...
//# sourceMappingURL=targeted-red-teaming.js.map