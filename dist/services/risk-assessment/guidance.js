...
exports.RISK_LEVEL_GUIDANCE = exports.RISK_CATEGORIES = void 0;
exports.getRiskAssessmentGuidance = getRiskAssessmentGuidance;
exports.getFieldGuidance = getFieldGuidance;
 ...
 */
async function getRiskAssessmentGuidance(category, riskLevel, regulationType, systemContext) {
    const categoryInfo = exports.RISK_CATEGORIES[category];
    const riskLevelInfo = riskLevel ? exports.RISK_LEVEL_GUIDANCE[riskLevel] : null;
    ...
    try {
        ...
        const platformContext = await (0, platform_rag_service_1.getPlatformContextString)(platformQuery, 5, 'risk-assessment' // Filter for risk assessment content if available
        );
        if (platformContext &&
        ...
        if (regulationType && regulationQuery) {
            ...
            const regulationContext = await (0, rag_service_1.getRegulationContextString)(regulationQuery, regulationType, 3);
 ...
 */
async function getFieldGuidance(field, category, riskLevel) {
    ...
    try {
        ...
        const platformContext = await (0, platform_rag_service_1.getPlatformContextString)(query, 2);
        if (platformContext &&
            platformContext !== 'No relevant platform knowledge found.' &&
            platformContext !== 'No query provided.') {
            return `${baseGuidance}\n\n**Additional Guidance:**\n${platformContext}`;
 ...
 */
function getStaticFieldGuidance(field, category, riskLevel) {
    const categoryInfo = exports.RISK_CATEGORIES[category];
    switch (field) {
        case 'summary':
 ...
 */
function extractBestPractices(text, category) {
    if (!text)
        return getDefaultBestPractices(category);
 ...
 */
function extractCommonPitfalls(text, category) {
    if (!text)
        return getDefaultCommonPitfalls(category);
 ...
 */
function getDefaultBestPractices(category) {
    switch (category) {
        case 'bias':
            return [
 ...
 */
function getDefaultCommonPitfalls(category) {
    switch (category) {
        case 'bias':
            return [
