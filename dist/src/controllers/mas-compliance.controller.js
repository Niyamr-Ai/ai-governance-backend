...
async function postMasCompliance(req, res) {
    try {
        ...
        Promise.resolve().then(() => __importStar(require("../../services/documentation/documentation-auto-generate"))).then(({ autoGenerateDocumentationIfNeeded }) => autoGenerateDocumentationIfNeeded(data.id, ["MAS"], userId))
            .catch(() => { });
        // Auto-generate automated risk assessment (non-blocking)
        axios_1.default
            .post(`http://localhost:3000/api/ai-systems/${data.id}/automated-risk-assessment`, {
