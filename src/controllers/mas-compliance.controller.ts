...
function sanitizeContent(content: string | null): any {
...
function mergeWithDefaults(result: any, answers: any, userId: string | null): Partial<MasAssessmentResult> {
  const getPillar = (key: string): MasPillarStatus => {
    const pillar = result?.[key];
    if (!pillar || typeof pillar !== "object") return defaultPillar();
    return {
      status:
  ...
  const complianceStatus =
    result?.overall_compliance_status === "Compliant" ||
  ...
  const rawAnswersData: Record<string, any> = {};
  ...
  return {
    ...
    uses_special_category_data: Boolean(
      result?.uses_special_category_data ?? answers.uses_special_category_data
    ),
  ...
  try {
    ...
    try {
      ...
      if (error.message?.includes("context length") || error.message?.includes("tokens")) {
        ...
        try {
          contextChunks = await getRegulationContextString(shortQuery, 'MAS', 5);
        } catch (retryError) {
...
`.trim();
    ...
    const payload = mergeWithDefaults(parsed, answers, userId) as any;
    ...
    if (error) {
      ...
      if (error.message?.includes("raw_answers")) missingColumns.push("raw_answers");
