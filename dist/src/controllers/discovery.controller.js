...
exports.createSmartAssessment = createSmartAssessment;
exports.getDiscovery = getDiscovery;
exports.postDiscovery = postDiscovery;
exports.getLinkSuggestions = getLinkSuggestions;
exports.markAsShadowAI = markAsShadowAI;
exports.resolveDiscoveredAsset = resolveDiscoveredAsset;
exports.createSystemFromAsset = createSystemFromAsset;
exports.getPrioritization = getPrioritization;
exports.linkDiscoveredAsset = linkDiscoveredAsset;
// Removed getUserId import - using req.user from auth middleware instead
const server_1 = require("../../utils/supabase/server");
const smart_shadow_ai_discovery_1 = require("../../services/compliance/smart-shadow-ai-discovery");
/**
 * POST /api/discovery/smart-assessment - Generate smart assessment for discovered AI assets
 */
async function createSmartAssessment(req, res) {
    try {
        ...
        const assessment = await (0, smart_shadow_ai_discovery_1.generateShadowAIAssessment)(asset, userId, organization_context);
        return res.json({
            assessment,
            asset: {
                id: asset.id,
 ...
 */
async function postDiscovery(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Unauthorized" });
 ...
 */
async function getLinkSuggestions(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
        ...
        const suggestions = await (0, smart_shadow_ai_discovery_1.suggestSystemLinks)(asset, userId, Math.min(Math.max(1, max_suggestions), 10) // Limit between 1-10
        );
        return res.json({
            suggestions,
            asset: {
 ...
 */
async function markAsShadowAI(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
 ...
 */
async function resolveDiscoveredAsset(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
 ...
 */
async function createSystemFromAsset(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
 ...
 */
async function getPrioritization(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
        ...
        const prioritizations = await (0, smart_shadow_ai_discovery_1.prioritizeDiscoveredSystems)(assets, userId);
 ...
 */
async function linkDiscoveredAsset(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "Authentication required" });
