...
export interface SystemComplianceData {
  ...
  owner?: string; // MAS specific
  risk_tier?: string; // EU AI Act: Prohibited, High-risk, Limited-risk, Minimal-risk
  compliance_status?: string; // Compliant, Partially compliant, Non-compliant
  prohibited_practices_detected?: boolean; // EU AI Act: true if prohibited practices found
  // UK AI principles compliance
...
export async function validateLifecycleTransition(
  ...
  complianceData: SystemComplianceData | null,
  ...
  if (newStage === 'Deployed') {
    return validateTransitionToProduction(
      complianceData,
 ...
 */
function validateTransitionToTesting(
 ...
 */
function validateTransitionToProduction(
  complianceData: SystemComplianceData | null,
  riskSummary: RiskAssessmentSummary
): LifecycleValidationResult {
  const errors: string[] = [];
 ...
 */
export function canEditRiskAssessment(
 ...
 */
export function canCreateRiskAssessmentInStage(
  lifecycleStage: LifecycleStage
): boolean {
 ...
 */
export function getLifecycleConstraints(
  lifecycleStage: LifecycleStage
): string[] {
