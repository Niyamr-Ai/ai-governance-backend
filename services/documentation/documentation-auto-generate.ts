...
import { getUserId } from "@/app/api/helper/auth";
import { OpenAI } from "openai";
import type { RegulationType } from "@/ai-governance-backend/types/documentation";
...
async function generateDocumentation(
  ...
  const riskSummary = riskAssessments.map(ra =>
    `- ${ra.category}: ${ra.risk_level} risk - ${ra.summary} (Mitigation: ${ra.mitigation_status})`
  ).join('\n');
...
export async function autoGenerateDocumentationIfNeeded(
  ...
  try {
    ...
    if (!userId) {
      console.error(`[Auto-Doc] No user ID found, skipping generation for system ${systemId}`);
      return;
    }
