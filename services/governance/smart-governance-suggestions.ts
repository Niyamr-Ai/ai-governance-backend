...
function mapRegulationTypeToRAG(regulationType: GovernanceRegulation): 'EU' | 'UK' | 'MAS' {
  return regulationType; // Direct mapping as they match
}

// Helper function to build RAG query for system context
function buildSystemContextQuery(context: TaskSuggestionContext): string {
...
function buildPlatformQuery(context: TaskSuggestionContext): string {
...
function generateSuggestionId(title: string): string {
...
function estimateEffort(category: SmartSuggestion['category'], priority: SmartSuggestion['priority']): SmartSuggestion['estimated_effort'] {
 ...
 */
export async function generateSmartGovernanceSuggestions(
...
- monitoring: Tasks for ongoing monitoring and oversight`
    ...
    const validSuggestions: SmartSuggestion[] = suggestions
 ...
 */
export async function getTaskContextualHelp(
  taskTitle: string,
 ...
 */
export async function analyzeTaskCompletionImpact(
  ...
  try {
    ...
    const platformContext = await getPlatformContextString(impactQuery, 3, 'governance');
