 ...
 */
export async function getUkCompliance(req: Request, res: Response) {
  try {
...
`.trim();
    ...
    if (isSoftwareOnly && assessment.sectorRegulation) {
      ...
      if (Array.isArray(assessment.sectorRegulation.requiredControls)) {
        assessment.sectorRegulation.requiredControls = assessment.sectorRegulation.requiredControls.filter((control: string) => {
      ...
      if (Array.isArray(assessment.sectorRegulation.gaps)) {
        assessment.sectorRegulation.gaps = assessment.sectorRegulation.gaps.filter((gap: string) => {
 ...
 */
export async function getUkComplianceById(req: Request, res: Response) {
  try {
    const userId = req.user!.sub;
    const { id } = req.params;
