 ...
 */
async function getDashboardInsights(systemsData, regulationType = 'EU') {
    ...
    let overallGuidance = '';
    let keyInsights = [];
    let riskTierGuidance = {};
    let complianceStatusGuidance = {};
    let nextSteps = [];
    try {
        // Get overall guidance from RAG
        console.log(`[Dashboard Insights] Querying ${regulationType} RAG for overall guidance`);
        const overallContext = await (0, rag_service_1.getRegulationContextString)(overallQuery, regulationType, 5);
        if (overallContext &&
 ...
 */
async function getSystemInsights(systemData, regulationType = 'EU') {
    ...
    try {
        ...
        const context = await (0, rag_service_1.getRegulationContextString)(systemQuery, regulationType, 3);
        if (context &&
 ...
 */
function analyzeRiskTiers(systemsData) {
    const counts = {
        prohibited: 0,
        high_risk: 0,
        limited_risk: 0,
 ...
 */
function analyzeComplianceStatus(systemsData) {
    const counts = {
        compliant: 0,
 ...
 */
function identifyCriticalSystems(systemsData) {
 ...
 */
function buildOverallGuidanceQuery(riskTierCounts, complianceStatusCounts, regulationType) {
 ...
 */
function buildCriticalSystemsQuery(criticalSystems, regulationType) {
    if (criticalSystems.length === 0)
        return '';
    const issues = criticalSystems.map(system => {
        const issues = [];
        if (system.prohibited_practices_detected)
            issues.push('prohibited practices');
        if (!system.high_risk_all_fulfilled)
            issues.push('missing high risk obligations');
        if (system.compliance_status === 'non_compliant')
 ...
 */
function buildSystemSpecificQuery(systemData, regulationType) {
 ...
 */
async function getRiskTierGuidance(riskTierCounts, regulationType) {
    const guidance = {};
    for (const [tier, count] of Object.entries(riskTierCounts)) {
        if (count > 0) {
            try {
                ...
                const context = await (0, rag_service_1.getRegulationContextString)(query, regulationType, 2);
 ...
 */
async function getComplianceStatusGuidance(statusCounts, regulationType) {
    const guidance = {};
 ...
 * Extract insights from RAG context
 */
function extractInsightsFromContext(context, regulationType, systemsData) {
    const insights = [];
    ...
    if (nonCompliantSystems.length > 0 && insights.length < 4) {
        ...
        const additionalCount = systemsData.filter(system => normalizeComplianceStatus(system.compliance_status) === 'non_compliant').length - nonCompliantSystems.length;
        insights.push({
            type: 'recommendation',
    ...
    if (prohibitedSystems.length > 0 && insights.length < 4) {
        ...
        const additionalCount = systemsData.filter(system => normalizeRiskTier(system.risk_tier || system.risk_level) === 'prohibited').length - prohibitedSystems.length;
        insights.push({
            type: 'warning',
    ...
    if (highRiskSystems.length > 0 && insights.length < 4) {
        ...
        const additionalCount = systemsData.filter(system => normalizeRiskTier(system.risk_tier || system.risk_level) === 'high_risk').length - highRiskSystems.length;
        insights.push({
            type: 'obligation',
 ...
 */
function extractNextStepsFromContext(context) {
    const steps = [];
    const lines = context.split('\n').filter(line => line.trim());
    lines.forEach(line => {
        const trimmed = line.trim();
 ...
 */
function extractActionsFromContext(context, systemData) {
    const actions = [];
    const lines = context.split('\n').filter(line => line.trim());
    lines.forEach(line => {
        const trimmed = line.trim();
 ...
 */
function generateFallbackInsights(riskTierCounts, complianceStatusCounts, systemsData) {
    const insights = [];
 ...
 */
function generateFallbackNextSteps(systemsData) {
    const steps = [];
    const criticalSystems = identifyCriticalSystems(systemsData);
    if (criticalSystems.length > 0) {
 ...
 */
function generateFallbackSystemInsights(systemData) {
    const insights = [];
    if (systemData.prohibited_practices_detected) {
        insights.push({
            type: 'warning',
 ...
 */
function generateFallbackActions(systemData) {
    const actions = [];
    if (systemData.prohibited_practices_detected) {
...
//# sourceMappingURL=dashboard-insights.js.map