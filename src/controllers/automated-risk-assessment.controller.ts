...
import type {
  ...
  CreateAutomatedRiskAssessmentInput,
 ...
 */
function mapUkRiskLevel(ukRiskLevel: string): 'Critical' | 'High' | 'Medium' | 'Low' {
  if (!ukRiskLevel) return 'Medium';
 ...
 */
function getComplianceRiskLevel(
  complianceData: any,
  hasUkData: boolean,
  hasMasData: boolean
 ...
 */
function generateComplianceChecklist(data: any): ComplianceChecklistItem[] {
  ...
  data.riskAssessments.forEach((ra: any, index: number) => {
    checklist.push({
...

{
  ...
  try {
    ...
    const formatObjectAsText = (obj: any, indent = 0): string => {
...
export async function createAutomatedRiskAssessment(req: Request, res: Response) {
  try {
    ...
    let normalizedRiskTier: string | undefined;
    ...
    const scoringData = {
      ...
      approved_risk_assessments: systemData.riskAssessments.map((ra: any) => ({
        category: ra.category,
        risk_level: ra.risk_level,
        mitigation_status: ra.mitigation_status,
      })),
      // Add framework-specific data for better scoring
      _framework: (systemData.hasEuData ? 'EU' : systemData.hasUkData ? 'UK' : 'MAS') as 'EU' | 'UK' | 'MAS',
    ...
    const scores = calculateRiskScores(scoringData, weights);
    ...
    const reportContent = await generateReportContent(
      { ...scores, overallRiskLevel: finalRiskLevel },
      scores.dimensionDetails,
      systemData
    );
    ...
    const dataSources = {
      ...
      risk_assessments: systemData.riskAssessments.map((ra: any) => ra.id),
