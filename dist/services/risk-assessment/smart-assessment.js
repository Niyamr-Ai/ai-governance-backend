...
exports.generateContextualRiskAssessment = generateContextualRiskAssessment;
exports.suggestRiskMitigations = suggestRiskMitigations;
...
function buildSystemRiskQuery(systemData) {
...
function buildRegulationRiskQuery(systemData, regulationType) {
...
function buildPlatformRiskQuery(systemData) {
 ...
 */
async function generateContextualRiskAssessment(systemData, userId, organizationHistory) {
    ...
    let contextQuality = { regulation: 'low', platform: 'low', system: 'low' };
    ...
    try {
        ...
        7, systemData.id, 'risk_assessment');
    ...
    try {
        ...
        platformContext = await (0, platform_rag_service_1.getPlatformContextString)(platformQuery, 7, 'risk_assessment');
 ...
 */
async function suggestRiskMitigations(riskFactors, systemContext, userId) {
    ...
    for (const riskFactor of riskFactors) {
        try {
            ...
            try {
                ...
                platformContext = await (0, platform_rag_service_1.getPlatformContextString)(platformQuery, 5, 'risk_assessment');
                if (platformContext === "No relevant platform knowledge found.")
                    platformContext = '';
            }
            catch (error) {
 ...
 */
async function analyzeRiskTrends(systemId, systemHistory, industryBenchmarks, userId) {
    ...
    try {
        ...
        if (userId) {
            try {
                platformContext = await (0, platform_rag_service_1.getPlatformContextString)('risk trend analysis AI systems risk management patterns trend prediction methodologies', 5, 'risk_assessment');
                if (platformContext === "No relevant platform knowledge found.")
                    platformContext = '';
            }
            catch (error) {
  ...
  "recommendations": [
    {
      ...
      "rationale": "why needed"
        ...
        if (latestScore < previousScore - 0.5)
...
//# sourceMappingURL=smart-assessment.js.map