...
interface SystemData {
  ...
  // Framework-specific metadata (for UK/MAS normalization)
  _framework?: 'EU' | 'UK' | 'MAS';
  _original_risk_level?: string;
}

const DEFAULT_WEIGHTS: RiskDimensionWeights = {
  technical: 0.2,
  operational: 0.2,
 ...
 */
function calculateTechnicalRisk(data: SystemData): { score: number; detail: DimensionDetail } {
  ...
  const technicalAssessments = data.approved_risk_assessments?.filter(
    (ra) => ['robustness', 'privacy', 'explainability'].includes(ra.category)
  ) || [];
 ...
 */
function calculateOperationalRisk(data: SystemData): { score: number; detail: DimensionDetail } {
  let score = 5; // Start at medium
  const findings: string[] = [];
  const gaps: string[] = [];
  const recommendations: string[] = [];
 ...
 */
function calculateLegalRegulatoryRisk(data: SystemData): { score: number; detail: DimensionDetail } {
  let score = 5; // Start at medium
  const findings: string[] = [];
  const gaps: string[] = [];
  const recommendations: string[] = [];
 ...
 */
function calculateEthicalSocietalRisk(data: SystemData): { score: number; detail: DimensionDetail } {
  let score = 5; // Start at medium
  const findings: string[] = [];
  const gaps: string[] = [];
  const recommendations: string[] = [];
  ...
  const biasAssessments = data.approved_risk_assessments?.filter(
    (ra) => ra.category === 'bias'
  ) || [];
  ...
  const explainabilityAssessments = data.approved_risk_assessments?.filter(
    (ra) => ra.category === 'explainability'
  ) || [];
  ...
  const technicalAssessments = data.approved_risk_assessments?.filter(
    (ra) => ['robustness', 'explainability'].includes(ra.category)
  ) || [];
 ...
 */
function calculateBusinessRisk(data: SystemData): { score: number; detail: DimensionDetail } {
  let score = 5; // Start at medium
  const findings: string[] = [];
  const gaps: string[] = [];
  const recommendations: string[] = [];
  ...
  const unmitigatedRisks = data.approved_risk_assessments?.filter(
    (ra) => ra.mitigation_status !== 'mitigated' && ra.risk_level === 'high'
  ).length || 0;
...
function calculateCompositeScore(
  ...
  weights: RiskDimensionWeights = DEFAULT_WEIGHTS
): number {
  const composite =
 ...
 */
function determineRiskLevel(compositeScore: number): RiskLevel {
  if (compositeScore >= 9) return 'Critical';
  if (compositeScore >= 7) return 'High';
...
export function calculateRiskScores(
  ...
  Object.keys(finalWeights).forEach((key) => {
    finalWeights[key as keyof RiskDimensionWeights] /= weightSum;
  });
