 ...
 */
function detectRegulationType(userMessage) {
 ...
 */
function detectRegulationTypeFromPageContext(pageContext) {
 ...
 */
function isRegulatoryQuestion(userMessage) {
    const message = userMessage.toLowerCase();
    // Regulatory keywords
    ...
    if (hasRegulatoryKeywords && !hasPlatformKeywords)
        return true;
    if (!hasRegulatoryKeywords && hasPlatformKeywords)
        return false;
 ...
 */
async function getExplainContext(userMessage, pageContext) {
    let regulatoryText = '';
    try {
        ...
        if (isRegulatory) {
            ...
            const ragContext = await (0, rag_service_1.getRegulationContextString)(userMessage, regulationType, 5);
            ...
            const platformContext = await (0, platform_rag_service_1.getPlatformContextString)(userMessage, 5);
 ...
 */
function computeConfidenceLevel(context) {
    ...
    const hasIncompleteData = !context.riskLevel ||
 ...
 */
async function getSystemAnalysisContext(userMessage, pageContext, userId) {
        ...
        try {
            ...
            if (euSystems.data) {
                euSystems.data.forEach((sys) => {
                    ...
                    if (sys.risk_tier)
                        system.riskLevels.push(`EU: ${sys.risk_tier}`);
                    if (sys.compliance_status)
                        system.complianceStatuses.push(`EU: ${sys.compliance_status}`);
                });
            }
            // Process UK systems
            if (ukSystems.data) {
                ukSystems.data.forEach((sys) => {
                    if (!systemMap.has(sys.id)) {
                        systemMap.set(sys.id, {
                            id: sys.id,
                            name: sys.system_name,
                    ...
                    if (sys.risk_level)
                        system.riskLevels.push(`UK: ${sys.risk_level}`);
                    if (sys.overall_assessment)
                        system.complianceStatuses.push(`UK: ${sys.overall_assessment}`);
                });
            }
            // Process MAS systems
            if (masSystems.data) {
                masSystems.data.forEach((sys) => {
                    if (!systemMap.has(sys.id)) {
                        systemMap.set(sys.id, {
                            id: sys.id,
                            name: sys.system_name,
                    ...
                    if (sys.overall_risk_level)
                        system.riskLevels.push(`MAS: ${sys.overall_risk_level}`);
                    if (sys.overall_compliance_status)
                        system.complianceStatuses.push(`MAS: ${sys.overall_compliance_status}`);
                });
            }
            ...
            if (ukSystems.data) {
                ukSystems.data.forEach((sys) => {
                    ...
                    if (status.includes('compliant') && !status.includes('non') && !status.includes('partial'))
                        ...
                        ukPartiallyCompliant++;
                    if (risk.includes('high') || risk.includes('critical') || risk.includes('frontier'))
                        ukHighRisk++;
                    else if (risk.includes('medium'))
                        ukMediumRisk++;
                    else if (risk.includes('low'))
...
${allSystems.length > 0 ? allSystems.map(s => {
    ...
    try {
        ...
        if (isComplianceQuery || isRiskAssessmentQuery) {
            ...
            if (masData) {
                ...
                if (userMsgLower.includes('governance') && (userMsgLower.includes('policy type') || userMsgLower.includes('framework') || userMsgLower.includes('policy'))) {
                    const rawAnswers = masData.raw_answers;
                    if (rawAnswers) {
                ...
                masPillars.forEach(pillar => {
                    const pillarData = masData[pillar];
            ...
            try {
                ...
                const userSystemContext = await (0, user_system_rag_service_1.getUserSystemContextString)(userMessage, userId, // TEMPORARY: using userId as orgId during transition
                5, systemId // Filter by specific system
                );
                    ...
                    if (regulationFromPage === 'MAS') {
                        ...
                        if (masData) {
                            ...
                            if (userMsgLower.includes('fairness')) {
                                const fairnessPillar = masData.fairness;
                                ...
                                const thirdPartyPillar = masData.thirdParty;
        ...
        if (isDocumentationQuery && systemId) {
            ...
            try {
                ...
                if (docError) {
                    ...
                    documentationRecords.forEach((doc, idx) => {
        ...
        if (isComparisonQuery && systemId && userId) {
            ...
            try {
                ...
                if (euSystemsResult.data) {
                    euSystemsResult.data.forEach((sys) => {
                ...
                if (ukSystemsResult.data) {
                    ukSystemsResult.data.forEach((sys) => {
                        if (sys.id !== systemId) { // Exclude current system
                            allSystems.push({
                                id: sys.id,
                ...
                if (masSystemsResult.data) {
                    masSystemsResult.data.forEach((sys) => {
                        if (sys.id !== systemId) { // Exclude current system
                            allSystems.push({
                                id: sys.id,
        ...
        try {
            ...
            const regulatoryContext = await (0, rag_service_1.getRegulationContextString)(userMessage, regulationType, 3);
...
async function getActionContext(userMessage, pageContext, userId) {
    try {
        ...
        try {
            ...
            const platformContext = await (0, platform_rag_service_1.getPlatformContextString)(userMessage + ' workflow steps actions', // Enhance query for workflow content
            5, 'workflows' // Filter for workflow-specific content if available
            );
            if (platformContext &&
                ...
                const stepLines = platformContext.split('\n').filter(line => line.toLowerCase().includes('next') ||
                ...
                if (isDashboardQuery) {
                    ...
                    if (allSystemIds.length > 0) {
                        ...
                        if (tasks && tasks.length > 0) {
                            ...
                            const systemIdToNameMap = new Map();
