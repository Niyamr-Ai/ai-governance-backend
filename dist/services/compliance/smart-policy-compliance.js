...
exports.analyzePolicyCompliance = analyzePolicyCompliance;
exports.identifyComplianceGaps = identifyComplianceGaps;
exports.detectPolicyConflicts = detectPolicyConflicts;
...
function buildPolicyComplianceQuery(policyName, policyDescription, systemName, systemDescription) {
...
function buildRegulationAlignmentQuery(policyName, regulationType, systemType) {
...
function buildPlatformComplianceQuery(policyType, complianceArea) {
 ...
 */
async function analyzePolicyCompliance(aiSystem, policies, userId) {
    ...
    for (const policy of policies) {
        try {
            ...
            try {
                const systemQuery = buildPolicyComplianceQuery(policy.name, policy.description, aiSystem.name, aiSystem.description);
            ...
            try {
                const platformQuery = buildPlatformComplianceQuery(policy.policy_type, 'compliance_assessment');
            ...
            try {
                const regulationQuery = buildRegulationAlignmentQuery(policy.name, regulationType, aiSystem.risk_level);
                regulationContext = await (0, rag_service_1.getRegulationContextString)(regulationQuery, regulationType, 5);
 ...
 */
async function identifyComplianceGaps(aiSystem, applicablePolicies, userId) {
    ...
    try {
        ...
        const platformContext = await (0, platform_rag_service_1.getPlatformContextString)(`compliance gap identification analysis ${aiSystem.risk_level} risk AI systems policy assessment methodologies`, 5, 'compliance');
 ...
 */
async function detectPolicyConflicts(policies, systemCharacteristics, userId) {
    ...
    try {
        ...
        const platformContext = await (0, platform_rag_service_1.getPlatformContextString)('policy conflict detection analysis governance frameworks compliance management conflict resolution', 5, 'compliance');
        // Build context for AI analysis
 ...
 */
function createBasicGapIdentification(aiSystem, applicablePolicies) {
    return {
        system_id: aiSystem.id,
        applicable_policies: applicablePolicies.map(policy => ({
            policy_id: policy.id,
            policy_name: policy.name,
            compliance_status: policy.current_compliance_status || 'not_assessed',
...
//# sourceMappingURL=smart-policy-compliance.js.map