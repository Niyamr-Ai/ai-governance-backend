 ...
 */
export function getAttacksByType(type: AttackType): AttackPrompt[] {
  return ATTACK_PROMPTS.filter(attack => attack.type === type);
}

/**
 * Get a random attack prompt
 */
export function getRandomAttack(): AttackPrompt {
  return ATTACK_PROMPTS[Math.floor(Math.random() * ATTACK_PROMPTS.length)];
}
