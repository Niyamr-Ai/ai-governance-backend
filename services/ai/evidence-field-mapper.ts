 ...
 */
export const EVIDENCE_FIELD_MAPPINGS: EvidenceFieldMapping[] = [
  // MAS Form - Governance
  {
 ...
 */
export function getEvidenceFieldMapping(evidenceKey: string): EvidenceFieldMapping | undefined {
  return EVIDENCE_FIELD_MAPPINGS.find(mapping => mapping.evidenceKey === evidenceKey);
}

/**
 * Check if an evidence key has auto-population support
 */
export function hasAutoPopulationSupport(evidenceKey: string): boolean {
  return EVIDENCE_FIELD_MAPPINGS.some(mapping => mapping.evidenceKey === evidenceKey);
}

/**
 * Get all evidence keys that support auto-population for a form type
 */
export function getSupportedEvidenceKeys(formType: 'MAS' | 'UK'): string[] {
  return EVIDENCE_FIELD_MAPPINGS
