...
export function calculateHistoryTokenBudget(
  model: keyof typeof MODEL_LIMITS = 'gpt-4o',
 ...
 */
export function truncateHistoryToFitBudget(
  history: Array<{ user_query: string; bot_response: string }>,
  tokenBudget: number
): Array<{ user_query: string; bot_response: string }> {
  if (tokenBudget <= 0) {
    return [];
  }

  const truncated: Array<{ user_query: string; bot_response: string }> = [];
  let currentTokens = 0;
 ...
 */
function truncateSingleEntry(
  entry: { user_query: string; bot_response: string },
  tokenBudget: number
): { user_query: string; bot_response: string } | null {
 ...
 */
export function truncateHistoryText(historyText: string, tokenBudget: number): string {
